...
/*var copService = require('../fabric-sdk-node/fabric-ca-client/lib/FabricCAClientImpl.js'); */
...
module.exports.CHAINCODE_PATH = 'github.com/chaincode';
...
module.exports.KVS = path.join(__dirname, '.hfc-test-kvs');
module.exports.storePathForOrg = function(org) {
	return module.exports.KVS + '_' + org;
};
// temporarily set $GOPATH to the test fixture folder
module.exports.setupChaincodeDeploy = function() {
	process.env.GOPATH = path.join(__dirname, '../../fabricsetup');
};
...
Client.addConfigFile(path.join(__dirname, '../fabricsetup/config.json'));
...
function getAdmin(client, t, userOrg) {
	var keyPath = path.join(__dirname, util.format('../../fabricsetup/crypto-config/peerOrganizations/org1.bicfujitsu.com/users/Admin@org1.bicfujitsu.com/msp/keystore', userOrg, userOrg));
	var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	var certPath = path.join(__dirname, util.format('../../fabricsetup/crypto-config/peerOrganizations/org1.bicfujitsu.com/users/Admin@org1.bicfujitsu.com/msp/signcerts', userOrg, userOrg));
...
function getOrdererAdmin(client, t) {
	var keyPath = path.join(__dirname, '../../fabricsetup/crypto-config/ordererOrganizations/example.com/users/Admin@bic.fujitsu.com/keystore');
	var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
	var certPath = path.join(__dirname, '../../fabricsetup/crypto-config/ordererOrganizations/example.com/users/Admin@bic.fujitsu.com/signcerts');
	var certPEM = readAllFiles(certPath)[0];
	t.comment('getOrdererAdmin');

	return Promise.resolve(client.createUser({
		username: 'BICordererAdmin',
		mspid: 'BICOrdererMSP',
		cryptoContent: {
			privateKeyPEM: keyPEM.toString(),
			signedCertPEM: certPEM.toString()
...
module.exports.getClientForOrg = async function(t, org) {
	...
	//  organization
	const client = Client.loadFromConfig('./fabricsetup/network-ad.yaml');
	...
	client.loadFromConfig('./fabricsetup/'+ org +'.yaml');
...
module.exports.setupChannel = async function(t, client_org1, client_org2, channel_name) {
	...
	try {
		...
		const envelope_bytes = fs.readFileSync(path.join(__dirname, '../../fabricsetup/channel-artifacts/channel1.tx'));
		...
		let request = {
			...
			name : channel_name,
			orderer : 'orderer.bic.fujitsu.com',
			txId  : tx_id
		};
		...
		let promises = [];
		let join_monitor = module.exports.buildJoinEventMonitor(t, client_org1, channel_name, 'peer0.org1.bicfujitsu.com');
		promises.push(join_monitor);

		tx_id = client_org1.newTransactionID(true);
		request = {
			targets: ['peer0.org1.bicfujitsu.com'],
			block : genesis_block,
			txId : 	tx_id
		};
		...
		promises = [];
		join_monitor = module.exports.buildJoinEventMonitor(t, client_org2, channel_name, 'peer0.org2.bicfujitsu.com');
		promises.push(join_monitor);

		tx_id = client_org2.newTransactionID(true);
		request = {
			targets: ['peer0.org2.bicfujitsu.com'],
			block : genesis_block,
			txId : 	tx_id
		};
		...
		request = {
			...
			args: ['a', '100', 'b', '200'],
			txId: tx_id,
			targets: ['peer0.org1.bicfujitsu.com','peer0.org2.bicfujitsu.com'],
		};
